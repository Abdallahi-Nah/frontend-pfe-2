...
async function handleNotesMatieresUpdate(updatedDoc, req) {
  try {
    ...
    // 2. Récupération initiale des documents liés
    const notesModules = await NotesModules.findOne({
      notesMatieres: updatedDoc._id,
    });
    if (!notesModules) throw new Error("Module de notes non trouvé");

    const resultatsSemestre = await ResultatsSemestres.findOne({
      notesModules: notesModules._id,
    });
    if (!resultatsSemestre) throw new Error("Résultats semestre non trouvé");

    const resultatsAnnee = await ResultatsAnneeUniversitaire.findOne({
      resultatsSemestres: resultatsSemestre._id,
    });
    if (!resultatsAnnee)
      throw new Error("Résultats année universitaire non trouvé");

    const matiere = await Matiere.findById(updatedDoc.matiere);
    if (!matiere) throw new Error("Matière non trouvée");

    const etudiant = await Etudiant.findById(updatedDoc.etudiant);
    if (!etudiant) throw new Error("Étudiant non trouvé");

    // 3. Calcul et mise à jour de la moyenne matière
    const tp = updatedDoc.tp ?? 0;
    const cc = updatedDoc.cc ?? 0;
    const noteEcritFinale = req.body.ratt ?? updatedDoc.ecrit ?? 0;

    const moyMat = calculMoyMatiere(matiere.hasTp, tp, cc, noteEcritFinale);
    updatedDoc.moyenne = moyMat;
    await updatedDoc.save();
    ...
    // 6. Mise à jour des décisions supérieures
    const decMod = await updateModuleDecision(notesModules);
    const decSem = await updateSemestreDecision(resultatsSemestre);
    const decAnn = await updateAnneeDecision(resultatsAnnee);
    ...
    console.error("Erreur dans handleNotesMatieresUpdate:", error.message);
    throw new Error(
