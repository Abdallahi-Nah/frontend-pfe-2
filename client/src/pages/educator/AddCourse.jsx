...
export default function AddCourse() {
  ...
  // Charger les emplois et le cours s'il existe
  useEffect(() => {
    fetchEmplois();
    getCourseById();
  }, []);
  ...
  const fetchEmplois = async () => {
    try {
      const response = await axios.get(
        `${backendUrl}/emploi/get/enseignant/${idEmp}`
      );
  ...
  const getCourseById = async () => {
    if (!idCourse) return;
    try {
      const response = await axios.get(`${backendUrl}/course/get/${idCourse}`);
      const course = response.data;
      setMatiere(course.matiere);
      setEmploi(course.emploi);
      setCourseTitle(course.courseTitle);
  ...
  // 🔁 Synchroniser automatiquement courseTitle avec le nom de la matière sélectionnée
  useEffect(() => {
    const fetchMatiereNom = async () => {
      if (matiere) {
        try {
          const res = await axios.get(`${backendUrl}/matiere/get/${matiere}`);
          if (res.data?.nom) {
            setCourseTitle(res.data.nom);
          }
        } catch (error) {
          console.error(
    ...
    // Mise à jour seulement si on est en création de cours
    if (!idCourse) {
      fetchMatiereNom();
    }
  }, [matiere]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (idCourse) {
        await axios.put(`${backendUrl}/course/update/${idCourse}`, data);
        alert("Cours modifié avec succès !");
      } else {
        await axios.post(`${backendUrl}/course/create`, data);
  ...
  return (
    <form className="p-4 max-w-2xl mx-auto" onSubmit={handleSubmit}>
      <div className="grid grid-cols-1 gap-4">
        ...
        >
          {idCourse ? "Modifier le cours" : "Créer le cours"}
        </button>
      </div>
    </form>
  );
