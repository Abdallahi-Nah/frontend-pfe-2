  ...
  return (
    <nav className="bg-white border-b border-gray-500 sticky top-0 z-50">
      <div className="flex items-center justify-between px-3 md:px-6 py-1">
        ...
        <div className="hidden md:flex items-center gap-2 text-gray-500">
          {token && (
            <div className="flex items-center gap-2">
              ...
              <img
                className="w-5 h-5 lg:w-6 lg:h-6 rounded-full object-cover shadow ring-1 ring-gray-300"
          ...
          {/* Desktop Auth Button - Très petit */}
          {!token
            ? !isLoginPage && (
                <Link
                  to="/login"
                  className="bg-blue-600 hover:bg-blue-700 text-white px-2 py-0.5 rounded-full cursor-pointer transition-colors duration-200 text-xs whitespace-nowrap"
                >
                  Log In
                </Link>
              )
            : !isLoginPage && (
                <button
                  ...
                  className="bg-red-600 hover:bg-red-700 text-white px-2 py-0.5 rounded-full cursor-pointer transition-colors duration-200 text-xs whitespace-nowrap"
                >
                  Log Out
                </button>
              )}
        ...
        {/* Mobile Menu Button - Très petit */}
        <button
          onClick={toggleMobileMenu}
          className="md:hidden flex flex-col items-center justify-center w-5 h-5 space-y-0.5 focus:outline-none"
          aria-label="Toggle mobile menu"
          aria-expanded={isMobileMenuOpen}
        >
          <span
            className={`block w-3 h-0.5 bg-gray-600 transition-transform duration-300 ${
              isMobileMenuOpen ? "rotate-45 translate-y-0.5" : ""
            }`}
          ></span>
          <span
            className={`block w-3 h-0.5 bg-gray-600 transition-opacity duration-300 ${
              isMobileMenuOpen ? "opacity-0" : ""
            }`}
          ></span>
          <span
      ...
      >
        <div className="px-3 py-1.5 space-y-1.5">
          {/* Mobile User Info - Très compact */}
          {token && (
            <div className="flex items-center gap-2 pb-1.5 border-b border-gray-200">
              <img
                className="w-6 h-6 rounded-full object-cover shadow ring-1 ring-gray-300"
                src={getUserProfile() || "/placeholder.svg"}
                alt={`Photo de profil de ${getUserName() || "utilisateur"}`}
              />
              <div>
          ...
          {!token
            ? !isLoginPage && (
                <Link
                  to="/login"
                  className="block w-full bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg cursor-pointer transition-colors duration-200 text-center font-medium text-xs"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Log In
                </Link>
            ...
            : !isLoginPage && (
                <button
                  ...
                  className="block w-full bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg cursor-pointer transition-colors duration-200 text-center font-medium text-xs"
                >
                  Log Out
                </button>
              )}
